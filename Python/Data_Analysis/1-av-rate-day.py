...
day_avarage = data.groupby(['Day']).mean()
...
def app():
    ...
    highcharts.options.title.text = "Average Rating by Day"
    
    # x = [3, 6, 8]
    # y = [4, 7, 8]
    # highcharts.options.series[0].data = list(zip(x.index, y)) is the same as:
    # highcharts.options.series[0].data = [[3, 4], [6, 7], [8, 9]]
    highcharts.options.xAxis.categories = list(day_avarage.index) 
    highcharts.options.series[0].data = list(day_avarage['Rating']) 
